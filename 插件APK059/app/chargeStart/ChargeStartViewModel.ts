...
export class ChargeStartViewModel extends BaseViewModel {
    ...
    //数据模块
    private mModel: ChargeStartModel;
    ...
    public constructor() {
        super('ChargeStart');
        this.mModel = new ChargeStartModel();
    }
     ...
     * 点击去充电
     */
    public onPressStartChargeStart = (): void => {
        console.log("viewmodel start 方法");
        this.mModel.startChargeStart();
    };

    /**
     * 点击结束去充电
     */
    public onPressStopChargeStart = (): void => {
    ...
    public onFinish = (event?: ComponentEvent): boolean => {
        this.mModel.finishRunning();
        if (event && event.status) {
            this.return_obj.command = "chargeAction";
