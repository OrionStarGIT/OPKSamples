...
//        matrix.setTranslate(0, 0);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        Log.d(TAG, "onTouchEvent mode: " + mMode + " mIsStraightLine: " + mIsStraightLine + "    mMode的内容===" + mMode);
...
//        }
        if (mMode == MapMode.COLLECT) {
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_DOWN:
                    Log.i("SHADOW_OPK", "MotionEvent.ACTION_DOWN" + MotionEvent.ACTION_DOWN);
                    lastPoint.set(event.getX(), event.getY());
                    break;
                case MotionEvent.ACTION_UP:
                    Log.i("SHADOW_OPK", "MotionEvent.ACTION_UP" + MotionEvent.ACTION_UP);
                    if (poseBeans != null) {
                        for (int i = 0; i < poseBeans.size(); i++) {
                            ...
                            if (isInRectf(rectf, event)) {
                                ...
                                LogTools.info("placeName"+placeName + "   POINT1点的位置===：" + Constant.NavigatorPoint.POINT1 + "   POINT2点的位置===：" + Constant.NavigatorPoint.POINT2);
...
//                                }
                ...
                case MotionEvent.ACTION_POINTER_DOWN:
                    Log.i("SHADOW_OPK", "MotionEvent.ACTION_POINTER_DOWN" + MotionEvent.ACTION_POINTER_DOWN);
                    isPinch = true;
                    lastSpace = pinchSpace(event);
                    lastRotate = pinchRotation(event);
                    lastPoint = pinchCenter(event);
                    break;
                case MotionEvent.ACTION_POINTER_UP:
                    Log.i("SHADOW_OPK", "MotionEvent.ACTION_POINTER_UP" + MotionEvent.ACTION_POINTER_UP);
                    isPinch = false;
                    break;
                case MotionEvent.ACTION_MOVE:
                    Log.i("SHADOW_OPK", "MotionEvent.ACTION_MOVE" + MotionEvent.ACTION_MOVE);
                    PointF point = new PointF(event.getX(), event.getY());
                    if (isPinch && mBitmap != null) {
                        float space = pinchSpace(event);
                        float rotate = pinchRotation(event);
